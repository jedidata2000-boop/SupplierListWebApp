...
// A(0):Photo  B(1):SKU  C(2):ItemName  D(3):Price  E(4):Qty  F(5):Units  G(6):CBM
// H(7):Cartons  I(8):ItemCost  J(9):SellingPrice  K(10):Date
...
function extractImageUrl(formulaOrValue) {
  ...
  // Match =IMAGE("url") or =IMAGE("url",1) or =IMAGE("url", 1) etc.
  const m = String(formulaOrValue).match(/=IMAGE\s*\(\s*"([^"]+)"/i);
  return m ? m[1] : '';
}
...
async function ensureSupplierSheet_(spreadsheetId, name) {
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId, range: `${name}!A1:K1`, valueInputOption: 'RAW',
    requestBody: {
      values: [['Photo', 'SKU', 'Item Name', 'Price (¥)', 'Qty/Carton', 'Units', 'CBM',
        'Ordered Cartons', 'Item Cost (Rp)', 'Selling Price (Rp)', 'Ordered Date']]
    }
  });
  ...
  await sheets.spreadsheets.batchUpdate({
    ...
    requestBody: {
      requests: [
        headerFormatRequest(sheetId, 11),
        {
          repeatCell: {
...
async function getOrders(spreadsheetId, supplierName) {
  ...
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId, range: `${supplierName}!A2:K`,
  ...
  return rows
    .map((row, i) => ({
      ...
      orderedCartons: parseInt(row[7]) || 0,
      itemCost:       parseInt(row[8]) || 0,
      sellingPrice:   parseInt(row[9]) || 0,
      orderedDate:    formatDateCell_(row[10])
    }))
    .filter(o => o.sku.trim())
...
async function addOrder(spreadsheetId, data) {
  ...
  const res = await sheets.spreadsheets.values.append({
    spreadsheetId, range: `${data.supplierName}!A:K`,
    valueInputOption: 'RAW', insertDataOption: 'INSERT_ROWS',
    requestBody: {
      values: [[
        '', data.sku, data.itemName, data.price, data.qtyPerCarton, data.units || '',
        data.cbm, data.orderedCartons, data.itemCost, data.sellingPrice, data.orderedDate
      ]]
    }
  });
...
async function updateOrder(spreadsheetId, data) {
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId, range: `${data.supplierName}!A${data.rowIndex}:K${data.rowIndex}`,
    valueInputOption: 'USER_ENTERED',
    requestBody: {
      values: [[
        photoCell, data.sku, data.itemName, data.price, data.qtyPerCarton, data.units || '',
        data.cbm, data.orderedCartons, data.itemCost, data.sellingPrice, data.orderedDate
      ]]
    }
  });
...
async function addOrdersBulk(spreadsheetId, orders) {
  ...
  const rows = orders.map(o => [
    '', o.sku, o.itemName, o.price, o.qtyPerCarton, o.units || '',
    o.cbm, o.orderedCartons, o.itemCost, o.sellingPrice, o.orderedDate
  ]);
  const res = await sheets.spreadsheets.values.append({
    spreadsheetId, range: `${supplierName}!A:K`,
    valueInputOption: 'RAW', insertDataOption: 'INSERT_ROWS',
...
async function uploadToImgBB(base64Image, name) {
  const apiKey = process.env.IMGBB_API_KEY;
  if (!apiKey) throw new Error('IMGBB_API_KEY is not set. Get a free API key at imgbb.com and add it to Netlify env vars.');
  const body = new URLSearchParams({ key: apiKey, image: base64Image, name }).toString();
    ...
    const req = https.request({
      ...
      res.on('end', () => {
        try {
          ...
          else reject(new Error('ImgBB: ' + (json.error && json.error.message || 'upload failed')));
        } catch (e) { reject(e); }
      });
    });
    req.on('error', reject);
...
async function uploadPhoto(spreadsheetId, data) {
  const { supplierName, rowIndex, imageBase64 } = data;

  const imageUrl = await uploadToImgBB(imageBase64, `${supplierName}_row${rowIndex}`);
...
async function searchBySku(spreadsheetId, sku) {
  ...
  const ranges = orderSheets.map(p => `${p.title}!A2:K`);
