...
// G(6):Cartons  H(7):ItemCost  I(8):SellingPrice  J(9):Date
...
async function getOrders(spreadsheetId, supplierName) {
  ...
  return rows
    .map((row, i) => ({
      ...
      itemCost:       parseInt(row[7]) || 0,
      sellingPrice:   parseInt(row[8]) || 0,
      orderedDate:    formatDateCell_(row[9])
    }))
    .filter(o => o.sku.trim())
  ...
  const res = await sheets.spreadsheets.values.append({
    ...
    requestBody: {
      values: [[
        ...
        data.orderedCartons, data.itemCost, data.sellingPrice, data.orderedDate
      ]]
    }
  });
...
async function addOrdersBulk(spreadsheetId, orders) {
  ...
  const rows = orders.map(o => [
    ...
    o.orderedCartons, o.itemCost, o.sellingPrice, o.orderedDate
  ]);
  const res = await sheets.spreadsheets.values.append({
